...
/*
 * Answer to 6.2
 * 利点： enum定数をメソッドの引数とする場合、型が文字列ではなくenum定数に限定される。
 */
public class Vehicle {
	...
	public void turn(Instruction rotation) {
		if (rotation.equals(Instruction.TURN_LEFT)) {
			direction -= 90;
		} else if (rotation.equals(Instruction.TURN_RIGHT)) {
			direction += 90;
		} else {
			System.out.println("Use Instruction.TURN_LEFT or Instruction.TURN_RIGHT.");
		}
	}
