...
public class BatteryTest {
	...
	public void testClone() {
		...
		try {
			battery1 = new Battery(max, current);
		} catch (Exception e) {
			e.printStackTrace();
			fail("Caught an unexpected exception.");
		}
		
		Battery battery2 = battery1.clone();
		assertTrue(battery1 instanceof Battery);
		assertTrue(battery2 instanceof Battery);
		assertEquals((Double)max, (Double)battery1.getMaxAmountl());
		assertEquals((Double)max, (Double)battery2.getMaxAmountl());
		assertEquals((Double)current, (Double)battery1.getCurrentAmount());
		assertEquals((Double)current, (Double)battery2.getCurrentAmount());
	}
